...
def get_plane(point):
    L = torch.cat([point[:, 1:, :], point[:, :1, :]])
    M = point
    R = torch.cat([point[:, :-1, :], point[:, -1:0, :]])
    X21 = M - L
    X31 = R - L
    A = X21[:, :, 1] * X31[:, :, 2] - X21[:, :, 2] * X31[:, :, 1]
    B = X21[:, :, 2] * X31[:, :, 0] - X21[:, :, 0] * X31[:, :, 2]
    C = X21[:, :, 0] * X31[:, :, 1] - X21[:, :, 1] * X31[:, :, 0]
    D = -torch.mul(L[:, :, 0], A) - torch.mul(L[:, :, 1], B) - torch.mul(L[:, :, 2], C)
    plane = torch.stack([A, B, C, D])
    return plane


def distance(self, point, plane, norm=True):
    if norm:
        norm = torch.sqrt(torch.sum(plane[:, :, :3] ** 2, dim=2)).reshape(plane.shape[0], plane.shape[1], 1)
        plane = plane / norm
    point_mult = torch.cat([point[:, :, :3], torch.ones(point.shape[0], point.shape[1], 1).cuda()], dim=2)
    leng = torch.bmm(point_mult, point.permute(0, 2, 1))
...
class cls_plane_module_rotate_point(cls_module):
    ...
    def add_plane(self, input):
        input['plane'] = get_transformlist(input['plane_point'])
        print('new plane distance', torch.max(distance(input['plane_point'], input['plane'], False)))  # distance
        return input

    def loss_plane_rotate(self, input, output):
        output['plane'] = get_plane()

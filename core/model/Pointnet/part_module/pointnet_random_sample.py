...
# F0016_DI02WH_F3D.obj is wrong; (maybe eps not okay)
class PointNetMSGRandomPReLU(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list, final_list, BatchNorm2d=nn.BatchNorm2d):
        super(PointNetMSGRandomPReLU, self).__init__()
        self.npoint = npoint
        self.radius_list = radius_list
        self.nsample_list = nsample_list
        self.conv_blocks = nn.ModuleList()
    ...
    def forward(self, xyz, features=None, static=None):
        ...
        if self.npoint != xyz.shape[1]:
            ...
            if static is not None:
                static = index_points(static, fpx_idx)
        else:
            new_xyz = xyz
        # print(new_xyz, new_xyz.shape)
        new_features_list = []
        ...
        # print(new_features.shape, new_xyz.shape)
        if static is not None:
            return new_xyz, new_features, static
        return new_xyz, new_features


class PointNetFeaturePReLU(nn.Module):
    def __init__(self, in_channel, mlp, fc, dropout, BatchNorm2d=nn.BatchNorm2d, BatchNorm1d=nn.BatchNorm1d):
        super(PointNetFeaturePReLU, self).__init__()
        self.convs = []
        last_channel = in_channel + 3
...
class PointNetPropagationNear(nn.Module):
    def __init__(self, in_channel, mlp):
        super(PointNetPropagationNear, self).__init__()
        last_channel = in_channel
        self.mlp = []
        for out_channel in mlp:
            self.mlp.append(nn.Conv1d(last_channel, out_channel, 1))
            self.mlp.append(nn.BatchNorm1d(out_channel))
            #self.mlp.append(nn.PReLU(out_channel))
            last_channel = out_channel

...
def compute_bbox_loss(end_points, config, config_matcher, loss_weight_dict):
    ...
    size_class_loss, size_giou_loss = [], []
    ...
    for bs in range(B):
        ...
        if GT_COUNT == 0:  # append zero
            ...
            size_class_loss.append(torch.zeros(MAXQ, GT_COUNT).type_as(batch_gt_center))
            size_giou_loss.append(0)
            continue
        batch_pred_center = pred_center[bs, :, None, :].repeat(1, GT_COUNT, 1)
        # size_class_loss
        batch_gt_class = size_class_gt[bs, None, box_label_mask[bs]].repeat(MAXQ, 1)
        batch_pred_class = size_class_pred[bs, :, None, :].repeat(1, GT_COUNT, 1)
        criterion = nn.CrossEntropyLoss(reduction='none')
        batch_class_loss = criterion(batch_pred_class.reshape(MAXQ*GT_COUNT, num_class), batch_gt_class.reshape(MAXQ*GT_COUNT))
        ...
        # print(meansize_gt.shape, ' << meansize gt') # I think it is right; normalize
        ...
        batch_pred_residual = size_residual_pred[bs, :, None, :, :].repeat(1, GT_COUNT, 1, 1)
        batch_gt_class_inds = batch_gt_class.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 1, C)
        batch_pred_residual = torch.gather(batch_pred_residual, 2, batch_gt_class_inds).squeeze(2)

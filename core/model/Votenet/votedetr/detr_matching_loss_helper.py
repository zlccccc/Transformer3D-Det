...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    nn_dist = nn_distance(pred_center, gt_center)
    center_dist = nn_dist[box_label_mask]
    metric['center_dist'] = center_dist
    ...
    heading_residual_normalized_loss = huber_loss(
        torch.sum(end_points['heading_residuals_normalized']*heading_label_one_hot, -1) - heading_residual_normalized_label, delta=1.0)  # (B,K)
